...
#include "../IOConverters/TtoPPOpenLConverter.h"
#include <stdint.h>
#include "../AbstractFramework.h"
#include "GravityInertiaSystemStatus.h"

//class that tries to account for gravity during axis motion. Designed to act as a supporting IOConverter to another IOConverter rather than
//usually directly controlling a axis itself.
...
class GravityCompensator: public IOConverter
{
  ...
  public:
    ...
    //        motorType: What kind of motor type is attached to this axis, in the TorqueConverterMotorTypes enum
    ...
    //        axis_id:  The ID of the axis in the system, starting from 1 and going up. What axis ID correlates to what role in reality
    //                   is defendant on GravInertSysStatus; see it for more details.
    GravityCompensator(GravityInertiaSystemStatus* sysStatus, TorqueConverterMotorTypes motor_type, float Kt, int motResistance_milliOhms, int staticMillivolts, uint8_t axis_Id);
    ...
    //        axis_id:  The ID of the axis in the system, starting from 1 and going up. What axis ID correlates to what role in reality
    //                   is defendant on GravInertSysStatus; see it for more details.
    GravityCompensator(GravityInertiaSystemStatus* sysStatus, TorqueConverterMotorTypes motor_type, float Kt, int motResistance_milliOhms, FeedbackDevice* fdev, uint8_t axis_Id);
