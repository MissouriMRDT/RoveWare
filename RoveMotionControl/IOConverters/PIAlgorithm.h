...
class PIAlgorithm : public IOConverter
{
  ...
  private:
    ...
    float deg_deadBand;//when the axis is within this many degrees of its destination, it stops
    ...
    //motor is allowed to move at when it's not simply stopping. This is because on most axiss, the motor probably won't even move
    ...
    //hardStops one and two track physical stops that the axis can't move through like walls.
    ...
    //
    // Inputs:  ret_OutputFinished: a return-by-reference. If true, then the axis has reached its desired position.
  ...
  public:
    ...
    //overview: function for specifying positions of hard stops attached to this axis,
    //          that is positions in degrees that the axis can't travel through.
    //          Use this to keep the axis from smashing into another system, for instance.
